...
import { createBBox, parseAnalysisResponse, formatVisionContext, stripMarkdownFences, mimeTypeForPath, buildDataUrl, primitivesAnalysisPrompt, normalizeContent, } from "./bridge.js";
describe("BBox", () => {
...
describe("mimeTypeForPath", () => {
    ...
    it("detects jpeg case-insensitive", () => assert.equal(mimeTypeForPath("x.JPEG"), "image/jpeg"));
    it("detects gif", () => assert.equal(mimeTypeForPath("x.gif"), "image/gif"));
    it("detects webp", () => assert.equal(mimeTypeForPath("x.webp"), "image/webp"));
    it("returns undefined for unknown", () => assert.equal(mimeTypeForPath("x.pdf"), undefined));
});
...
describe("primitivesAnalysisPrompt", () => {
    ...
    it("produces a parseable JSON shape", () => {
        ...
        assert.doesNotThrow(() => JSON.parse(block), "prompt shape must be valid JSON");
    });
});
