...
export function createBBox(
	...
	if (x1 > NORM_MAX || y1 > NORM_MAX || x2 > NORM_MAX || y2 > NORM_MAX) {
		throw new Error(
			`coordinates must be 0..${NORM_MAX}, got (${x1},${y1},${x2},${y2})`,
		);
	}
	return { x1, y1, x2, y2 };
}
...
export function primitivesAnalysisPrompt(): string {
	return (
		`Analyze this image for another text-only model.\n` +
		`Return only one valid JSON object. Do not wrap it in Markdown.\n` +
		`Use this exact shape:${NOTE_JSON_SHAPE}\n` +
...
function analysisUserMessage(desc: string, question?: string): string {
 ...
 */
export function parseAnalysisResponse(raw: string): VisionAnalysis {
...
function parseNoteFromValue(json: unknown): ImageNote {
	const root = json as Record<string, unknown>;
	// bridge.rs: `let o = json.get("note").unwrap_or(json);`
	const o = (root.note as Record<string, unknown> | undefined) ?? root;
...
function parsePrimitivesFromValue(json: unknown): VisualPrimitive[] {
	const root = json as Record<string, unknown>;
	...
	return items
		...
		.filter((p): p is VisualPrimitive => p !== null);
}
...
function normalizePrimitive(
	...
	const id =
		typeof obj.id === "string" && obj.id.length > 0 ? obj.id : `v${index + 1}`;
...
export function buildDataUrl(mime: string, bytes: Uint8Array | Buffer): string {
	const base64 =
...
export function formatVisionContext(analysis: VisionAnalysis): string {
	...
	if (analysis.primitives.length === 0) {
		...
		for (const prim of analysis.primitives) {
			parts.push(
				`- ${prim.id} | type: ${prim.type} | box: [${prim.box.x1},${prim.box.y1},${prim.box.x2},${prim.box.y2}] | ref: ${prim.label} | confidence: ${prim.confidence.toFixed(2)}`,
			);
		}
	}
